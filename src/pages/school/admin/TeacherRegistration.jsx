...
export default function TeacherRegistration() {
  ...
  useEffect(() => {
    ...
    async function fetchTeacher() {
      ...
      try {
        ...
        handleApiError(error, 'Failed to load teacher details');
  ...
  const goBack = () => navigate('/school/admin/teachers');

  const handleSubmit = async (formData) => {
    setIsSubmitting(true);
    try {
      if (isEditMode) {
        await api.put(`/teachers/${id}`, mapTeacherFormToApiUpdate(formData));
        toast.success('Teacher updated successfully');
      } else {
        await api.post('/teachers', mapTeacherFormToApi(formData));
        const tenant = institute?.tenantDomain || localStorage.getItem('tenantDomain') || 'demo-school';
        toast.success(`Teacher created successfully. Portal: ${getTenantLoginUrl(tenant)}`);
      }
