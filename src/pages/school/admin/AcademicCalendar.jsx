          ...
          <div className="grid gap-0 lg:grid-cols-[1fr_340px]">
            <div className="border-b border-slate-100 lg:border-b-0 lg:border-r lg:border-slate-100">
              {loading || metaLoading ? (
                ...
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {monthDays.map((day, index) => {
                      if (!day) return <div key={`empty-${index}`} className="min-h-40 rounded-3xl border border-dashed border-slate-100 bg-slate-50/40" />;
                ...
                <div className="grid min-h-[760px] grid-cols-7 gap-0">
                  {weekDays.map((day) => {
                    const dayEvents = filteredEvents.filter((event) => sameDay(event.startTime, day) || isWithinRange(event, day));
                    const isToday = sameDay(day, new Date());
                    return (
                    ...
                    {agendaDays.map((day) => {
                      ...
                      return (
                        <div key={dateKey(day)} className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm">
                          ...
                          <div className="space-y-2">
                            {dayEvents.map((event) => (
                              <button key={event.id} draggable onDragStart={() => setDragId(event.id)} onClick={() => openEdit(event)} className="flex w-full items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-left">
                                <div>
