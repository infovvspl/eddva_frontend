  ...
  useEffect(() => {
    ...
    if (!socketConnected) {
      interval = setInterval(() => {
        console.info('[ChatSystem] Socket offline, running fallback polling...');
        void loadActiveTabRef.current(false);
        if (activeContactRef.current) {
          void fetchMessagesRef.current(activeContactRef.current.id);
        }
      }, 30000); // 30 seconds
    }
    return () => {
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center justify-end gap-1 text-[9px] opacity-85">
                          ...
                          {mine && (
                            <span>
                              {msg.is_read ? (
                                <CheckCheck size={12} className="text-blue-600 inline" />
                              ) : msg.is_delivered ? (
                                <CheckCheck size={12} className="text-slate-400 inline" />
                              ) : (
                                <Check size={12} className="text-slate-400 inline" />
                              )}
                            </span>
                          )}
                        </div>
