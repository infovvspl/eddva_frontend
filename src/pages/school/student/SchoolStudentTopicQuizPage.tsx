...
import {
  ...
  Target, ArrowLeft, Flame, Sparkles,
  Brain, Trophy, Play, Monitor, Zap,
...
// ─── Quiz Runner ──────────────────────────────────────────────────────────────
function QuizRunner({
  title, questions, seconds, timerDanger,
...
// ─── Main Component ───────────────────────────────────────────────────────────
type Stage = "loading" | "error" | "ai_generating" | "ai_info" | "ai_quiz" | "ai_submitting" | "ai_results";

export default function SchoolStudentTopicQuizPage() {
  ...
  const durationSec = (aiQuizData?.durationMinutes ?? 15) * 60;
  const isRunning = stage === "ai_quiz";
  ...
  const handleAiSelect = (optionId: string) => {
    ...
    const next = isMulti
      ? (answers[q.id]?.includes(optionId) ? answers[q.id].filter(id => id !== optionId) : [...(answers[q.id] || []), optionId])
      : [optionId];
  ...
  const handleAiSubmit = async (timedOut: boolean) => {
    ...
    try {
      ...
      const totalMarks = aiQuizData.totalMarks;
      const accuracy = qs.length ? (correct / qs.length) * 100 : 0;
  ...
  const handleCloseQuiz = async () => {
    const ok = await confirm({
      title: "Exit Quiz?",
      message: "Close this quiz? You will lose current progress and have to restart.",
      confirmLabel: "Exit",
      cancelLabel: "Resume",
    });
    if (ok) navigate(-1);
  ...
  if (stage === "ai_results" && aiResult && aiQuizData) {
    const correctCount = Math.round((aiResult.accuracy / 100) * questions.length);
