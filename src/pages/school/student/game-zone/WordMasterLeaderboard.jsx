...
import { Trophy, ArrowLeft, Loader2, Star, BookOpen, Zap } from 'lucide-react';
import { toast } from 'sonner';

export default function WordMasterLeaderboard({ onBack }) {
  ...
  return (
    <div className="space-y-6 max-w-xl mx-auto py-8">
      ...
      <div className="text-center space-y-2">
        ...
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">Vocab Wizards</h1>
        <p className="text-sm font-medium text-slate-500">Top scoreboard rankings for Word Master decks.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        {rankings.length === 0 ? (
          <div className="text-center py-10 text-slate-400">
            ...
            <p className="text-sm font-bold text-slate-500">No Word Master scores submitted yet.</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-right">
                    <div>
                      <p className="text-sm font-black text-slate-950 dark:text-white flex items-center gap-1 justify-end text-violet-650 dark:text-violet-400">
                        <Star className="h-3.5 w-3.5 fill-current" /> {user.score} XP
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-0.5">
                        {user.correctAnswers}/{user.wordsAttempted} words • <span className="text-violet-550 dark:text-violet-300 font-black">{user.maxStreak} streak</span>
                      </p>
                    </div>
                  </div>
                </div>
