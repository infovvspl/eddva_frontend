  ...
  return (
    <div className="space-y-6 max-w-xl mx-auto py-8 animate-fade-in">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400 border border-emerald-500/20 shadow-xl">
          <Medal className="h-8 w-8" />
        </div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white">Deck Cleared!</h1>
        <p className="text-sm font-semibold text-slate-500">You matched all the card pairs. Here is your loot scorecard:</p>
      </div>
      ...
      {badgeUnlocked && (
        <div className="rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 via-white to-indigo-50/50 p-5 dark:border-indigo-900/40 dark:from-indigo-950/20 dark:to-slate-950 shadow-md flex items-center gap-4 animate-pulse">
          ...
          <div>
            ...
            <h3 className="text-base font-black text-slate-950 dark:text-white">🏆 Badge Unlocked: Mind Matcher</h3>
            <p className="text-xs font-semibold text-slate-500">Congratulations on clearing a Hard Memory Match deck in under 20 turns!</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-950/50 flex flex-col items-center justify-center">
            <Star className="h-6 w-6 text-emerald-500 fill-current mb-1.5" />
        ...
        <div className="mt-6 border-t border-slate-100 dark:border-slate-800 pt-5 space-y-3">
          <div className="flex items-center justify-between text-sm font-black">
            <span className="text-slate-400">Total turns taken</span>
            <span className="text-slate-800 dark:text-white font-mono">{turnsCount} turns</span>
          </div>

          <div className="flex items-center justify-between text-sm font-black">
            <span className="text-slate-400">Mismatches made</span>
            <span className="text-rose-600 dark:text-rose-450 font-mono">{mismatchesCount}</span>
          </div>

          <div className="flex items-center justify-between text-sm font-black">
            <span className="text-slate-400">Match Accuracy</span>
            <span className={`px-2 py-0.5 rounded-md text-xs font-black font-mono ${
              matchAccuracy >= 70 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-300' :
              matchAccuracy >= 40 ? 'bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-300' :
      ...
      <div className="grid gap-2">
        <button
          onClick={onPlayAgain}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3 text-sm font-black text-white hover:bg-emerald-700 shadow-md transition"
        >
        ...
        >
          <Trophy className="h-4 w-4 text-amber-500" /> View Rankings Leaderboard
        </button>
      </div>
