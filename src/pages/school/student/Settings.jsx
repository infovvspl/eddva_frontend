  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs sm:text-sm font-medium text-slate-500">Manage theme, mobile alerts, password, and device sessions.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 sm:mt-5 grid gap-2.5 grid-cols-2">
              {['light', 'dark'].map((option) => (
                <button
                  key={option}
                  type="button"
                  onClick={() => setTheme(option)}
                  className={`flex items-center justify-between rounded-xl border p-3 sm:p-4 text-left transition ${
                    theme === option
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 sm:mt-5 space-y-2.5 sm:space-y-3">
              {[
                ...
                ['assignmentReminders', 'Assignment Reminders', 'Due date reminders.'],
                ...
                <button
                  ...
                  className="flex w-full items-center justify-between gap-3 rounded-xl border border-slate-200 p-3 sm:p-4 text-left hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800"
                >
                  <span>
                    <span className="block text-xs sm:text-sm font-black text-slate-950 dark:text-white">{label}</span>
