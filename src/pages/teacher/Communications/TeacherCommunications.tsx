...
export default function TeacherCommunications({ heightClass = 'flex-1 min-h-0 h-full' }) {
  ...
  useEffect(() => {
    ...
    const validPanelKeys = ['STUDENT', 'INSTITUTE_ADMIN'];
  ...
  return (
    <div className={`flex ${heightClass} min-h-0 w-full flex-col sm:pt-4 lg:pb-4 sm:px-6 pb-6 sm:pb-8`}>
      {!isSuperAdmin && (
        <div className="shrink-0 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
          {[
            ...
            <div key={idx} className="rounded-2xl border border-slate-200/80 bg-white p-3.5 shadow-sm hover:border-slate-300 transition-all">
              <p className="text-[11px] font-bold text-slate-600 uppercase tracking-wider">{item.label}</p>
              <div className="mt-1.5 flex items-baseline gap-2">
                <span className={`text-2xl font-black ${item.alert ? 'text-red-600' : 'text-slate-900'}`}>{item.val}</span>
      ...
      <div className="mt-[30px] mb-4 sm:mb-6 flex-1 min-h-0 overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-xl flex flex-col md:flex-row relative">
        ...
        <div className={`flex-1 flex flex-col min-w-0 min-h-0 bg-white ${
          !selectedUser
            ? 'hidden md:flex'
            : 'fixed inset-0 z-[100] bg-white flex flex-col md:relative md:inset-auto md:z-auto'
        }`}>
          {!selectedUser ? (
              ...
              <div className="flex items-center justify-between p-3 sm:p-4 border-b border-slate-100 bg-white shrink-0 shadow-xs z-10">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <button
                    ...
                    className="flex md:hidden items-center justify-center h-9 w-9 shrink-0 rounded-full bg-slate-100 text-slate-800 hover:bg-slate-200 transition active:scale-95 -ml-1"
