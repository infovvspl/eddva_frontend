...
export default function TeacherStudentDetailPage() {
  ...
  return (
    <div className="p-4 sm:p-6 w-full space-y-6 sm:space-y-8 animate-in fade-in duration-500 overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="card-surface p-5 sm:p-6 lg:p-8 relative overflow-hidden group border-primary/5 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 relative z-10 w-full">
              ...
              <div className="flex flex-row items-start sm:items-center gap-4 sm:gap-6 flex-1 min-w-0">
                <div className="relative shrink-0">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-3xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center text-lg sm:text-2xl md:text-3xl font-black text-primary border-2 border-primary/20 shadow-xl shadow-primary/10">
                    {getInitials(profile.name)}
                  </div>
                  {insights?.riskStatus === "critical" && (
                    <div className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 p-1.5 sm:p-2 bg-red-500 rounded-full border-4 border-card animate-pulse shadow-lg shadow-red-500/50">
                ...
                <div className="flex-1 min-w-0 space-y-2 sm:space-y-3">
                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-black text-foreground tracking-tight leading-tight break-words min-w-0">{profile.name}</h2>
                    {engagement && (
                      <Badge variant="outline" className={`gap-1.5 border-none ${engagement.color} bg-current/10 py-0.5 px-2.5 sm:py-1 sm:px-3 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider shrink-0`}>
                        <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-current animate-pulse" />
                        {engagement.label}
                      </Badge>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs sm:text-sm text-muted-foreground font-bold">
                    <div className="flex items-center gap-1 bg-muted/30 px-2.5 py-0.5 rounded-full border border-border/50">
                    ...
                    {profile.targetCollege && (
                      <div className="flex items-center gap-1 bg-primary/5 px-2.5 py-0.5 rounded-full border border-primary/10 text-primary/90">
              ...
              {/* Action Buttons */}
              <div className="flex flex-row sm:flex-col gap-2.5 sm:gap-3 shrink-0 w-full sm:w-auto">
                <Button 
                  variant="outline" 
                  className="flex-1 sm:flex-none justify-center rounded-full border-orange-500/20 text-orange-400 hover:bg-orange-500/10 hover:border-orange-500/40 h-9 sm:h-10 px-4 sm:px-6 font-black text-xs sm:text-sm transition-all duration-300 shadow-sm" 
                ...
                <Button 
                  variant="outline" 
                  className="flex-1 sm:flex-none justify-center rounded-full border-destructive/20 text-destructive hover:bg-destructive/10 hover:border-destructive/40 h-9 sm:h-10 px-4 sm:px-6 font-black text-xs sm:text-sm transition-all duration-300 shadow-sm" 
            ...
            {/* Bottom Stats Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3.5 pt-4 border-t border-border/40 relative z-10">
              <div className="flex flex-col items-center sm:items-start p-3.5 rounded-2xl bg-muted/20 border border-border/40 shadow-sm">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Current Streak</span>
              ...
              <div className="flex flex-col items-center sm:items-start p-3.5 rounded-2xl bg-muted/20 border border-border/40 shadow-sm">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Total XP</span>
                <span className="text-sm font-black text-foreground flex items-center gap-1.5">
                  <Zap className="w-4.5 h-4.5 text-yellow-500 fill-yellow-500/20" />
                  {profile.xpTotal.toLocaleString()}
                </span>
              </div>
              <div className="flex flex-col items-center sm:items-start p-3.5 rounded-2xl bg-muted/20 border border-border/40 shadow-sm">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Global Rank</span>
                <span className="text-sm font-black text-foreground flex items-center gap-1.5">
                  <Trophy className="w-4.5 h-4.5 text-blue-500 fill-blue-500/20" />
                  #{profile.rank?.toLocaleString() || "N/A"}
                </span>
              </div>
              <div className="flex flex-col items-center sm:items-start p-3.5 rounded-2xl bg-muted/20 border border-border/40 shadow-sm">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Level</span>
                <span className="text-sm font-black text-foreground flex items-center gap-1.5">
                  <ShieldCheck className="w-4.5 h-4.5 text-indigo-500 fill-indigo-500/20" />
                  {profile.level || 1}
                </span>
              </div>
              <div className="flex flex-col items-center sm:items-start p-3.5 rounded-2xl bg-muted/20 border border-border/40 shadow-sm col-span-2 sm:col-span-1">
                <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1.5">Presence</span>
                <span className="text-sm font-black text-foreground flex items-center gap-1.5">
          ...
          <TabsList className="bg-transparent h-auto p-0 gap-6 sm:gap-8 flex w-max min-w-full">
            {["overview", "performance", "engagement", "plan", "advanced"].map((t) => (
              <TabsTrigger 
                key={t} 
                value={t} 
                className="data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:shadow-none border-b-2 border-transparent data-[state=active]:border-primary rounded-none px-0 pb-3 text-xs sm:text-sm font-black capitalize tracking-tight shrink-0 transition-all"
              >
                {t === "plan" ? "Study Plan" : t}
              </TabsTrigger>
            ))}
