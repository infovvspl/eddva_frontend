...
import {
  ...
  User, Building2, BookOpen, Award, Monitor, Tag,
} from "lucide-react";
...
const CLASS_TYPE_OPTIONS = [
  ...
  { key: "offline", label: "Offline", icon: <Users className="w-4 h-4" /> },
  { key: "hybrid",  label: "Hybrid",  icon: <Zap className="w-4 h-4" /> },
] as const;

const TEACHING_MODE_OPTIONS = [
  { key: "live",     label: "Live Classes" },
  { key: "recorded", label: "Recorded Videos" },
  { key: "both",     label: "Live + Recorded" },
] as const;
...
function ProfileTab() {
  ...
  const toggleClassType = (key: string) => {
    setForm(f => ({
      ...f,
      classTypes: f.classTypes.includes(key)
        ? f.classTypes.filter(c => c !== key)
        : [...f.classTypes, key],
    }));
  };
  ...
  const addCourse = () => {
    ...
    if (!trimmed || form.coursesOffered.includes(trimmed)) return;
    setForm(f => ({ ...f, coursesOffered: [...f.coursesOffered, trimmed] }));
    setCourseInput("");
  };

  const removeCourse = (course: string) => {
    setForm(f => ({ ...f, coursesOffered: f.coursesOffered.filter(c => c !== course) }));
  };
      ...
      <div className="bg-card border border-border rounded-2xl p-6 flex items-center gap-5">
        <div className="relative shrink-0">
          ...
          <button
            ...
            className="absolute -bottom-1.5 -right-1.5 w-7 h-7 rounded-full bg-primary text-white flex items-center justify-center shadow hover:bg-primary/90 transition-colors"
          >
            {uploadImage.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
          </button>
          <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImagePick} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Organisation Image</p>
          <p className="text-sm text-muted-foreground">Click the upload button to change your institute's logo or profile picture.</p>
        </div>
      </div>

      {/* Grid: Institute name + Admin name */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="space-y-2">
          <label className="text-sm font-semibold text-foreground flex items-center gap-2">
            <Building2 className="w-4 h-4 text-primary" /> Institute Name
          </label>
          <input
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-foreground flex items-center gap-2">
            <User className="w-4 h-4 text-primary" /> Admin Name
          </label>
          <input
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3 flex-wrap">
          {CLASS_TYPE_OPTIONS.map(opt => {
            const active = form.classTypes.includes(opt.key);
            return (
              <button
                key={opt.key}
                onClick={() => toggleClassType(opt.key)}
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3 flex-wrap">
          {TEACHING_MODE_OPTIONS.map(opt => (
            <button
              key={opt.key}
              onClick={() => setForm(f => ({ ...f, teachingMode: opt.key }))}
