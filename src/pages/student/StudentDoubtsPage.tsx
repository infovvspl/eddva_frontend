...
const getQuestionSnippet = (text?: string | null) => {
  if (!text) return "";
  return text
...
function DoubtCard({ doubt }: { doubt: StudentDoubt }) {
  ...
  const rawQuestion = doubt.questionText || (doubt.questionImageUrl ? "Image question" : "No question text");
  const snippet = getQuestionSnippet(rawQuestion);
  const isLong = snippet.length > 100;
  const displayText = isLong && !expanded ? snippet.slice(0, 100) + "..." : snippet;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="font-semibold text-slate-800 text-sm">
            ...
            {isLong && !expanded && (
              <span className="text-indigo-600 font-bold ml-1.5 hover:underline cursor-pointer">
                more
              </span>
            )}
          </div>
          ...
          >
            <div className="px-5 pb-5 space-y-4 border-t border-slate-50 pt-4">
                ...
                <div className="bg-blue-50 rounded-xl border border-blue-100 overflow-hidden">
                  ...
                  <div className="flex flex-col gap-3 px-4 pt-4 pb-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center justify-between sm:justify-start gap-2 w-full sm:w-auto">
                      ...
                      {parsedAi?.subject && (
                        <span className="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-bold capitalize sm:ml-2">
                    ...
                    {parsedAi && (
                      <div className="flex gap-0.5 bg-blue-100/70 p-0.5 rounded-lg shrink-0 self-start sm:self-auto">
                        <button
              ...
              {canAskAi && !doubt.aiExplanation && (
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl">
                  {/* Icon + text info */}
                  ...
                  {/* Action controls (Brief/Detailed toggle + Ask AI button) */}
                  <div className="flex items-center justify-between sm:justify-end gap-2.5 w-full sm:w-auto flex-wrap">
                    <div className="flex gap-1.5 bg-indigo-100/50 p-1 rounded-xl shrink-0">
                      <button
                    ...
                    <button
                      ...
                      className="shrink-0 inline-flex items-center gap-1.5 px-4 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 justify-center flex-1 sm:flex-initial"
                    >
                      {requestAi.isPending
  ...
  return (
    <div className="w-full p-4 sm:p-6 pb-24 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex gap-1.5 overflow-x-auto pb-2 custom-scrollbar w-full sm:w-auto">
          {TABS.map(tab => (
            <button
              key={tab.key}
        ...
        <div className="flex items-center gap-1.5 shrink-0 px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs font-medium text-slate-600 w-full sm:w-48">
