...
export default function SlidePresenter({ imageUrls, onSlide, className }: SlidePresenterProps) {
  ...
  // Empty state — prompt to load a PDF. Styled for the dark Studio strip.
  if (total === 0) {
    return (
      ...
      <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
        {slides.map((s, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={`relative h-12 w-20 shrink-0 overflow-hidden rounded-lg border-2 transition ${i === index ? 'border-blue-500' : 'border-transparent opacity-60 hover:opacity-100'}`}
            title={`Slide ${i + 1}`}
          >
            <SlideThumb slide={s} />
