...
import { 
  ...
  Search, Check, X, Loader2, ChevronDown
} from 'lucide-react';
...
const MultiSelectDropdown = ({ label, options, selectedIds, onChange, placeholder }) => {
  ...
  return (
    <div className="relative group">
      <div 
        ...
        className="w-full min-h-[54px] rounded-2xl border-2 border-slate-100 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 px-4 py-3 flex items-center justify-between cursor-pointer transition-all hover:border-blue-500"
      >
        <div className="flex flex-wrap gap-1">
          {selectedItems.length > 0 ? (
            selectedItems.map(item => (
              <span key={item.id} className="px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs font-bold">
                {item.name}
              </span>
            ))
          ) : (
            <span className="text-slate-400 text-sm font-semibold">{placeholder}</span>
          )}
        </div>
        <ChevronDown size={18} className="text-slate-400" />
      </div>
      <label className="absolute left-4 -top-2.5 px-2 bg-white dark:bg-slate-900 text-[10px] font-bold text-blue-600 uppercase">
        {label}
      </label>

      {isOpen && (
        <div className="absolute z-50 top-full left-0 right-0 mt-2 max-h-60 overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl p-2 custom-scrollbar">
          {options.map(option => {
            const isSelected = selectedIds.includes(option.id);
            return (
              <label key={option.id} className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors" onClick={(e) => e.stopPropagation()}>
                <input 
                  type="checkbox"
                  checked={isSelected}
                  onChange={() => {
...
export default function AddTeacherMultiStep({ teacher, onSubmit, onCancel, isLoading }) {
  ...
  const [formData, setFormData] = useState({
    ...
    salary: '', employeeCode: '', classTeacher: '', classIds: [], sectionIds: [], subjectIds: [],
    shift: '', prevOrg: '',
  ...
  const renderProfessionalDetails = () => (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      ...
      <div className="mb-8">
        <h5 className="text-[10px] font-bold tracking-tight text-slate-400 uppercase tracking-widest mb-4 ml-1">Class, Section & Subject Assignments</h5>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <MultiSelectDropdown 
            label="Assign Classes" 
            options={classes} 
            selectedIds={formData.classIds || []} 
            onChange={(ids) => setFormData(prev => ({ ...prev, classIds: ids }))}
            placeholder="Select Classes..."
          />
          <MultiSelectDropdown 
            label="Assign Sections" 
            options={classes.flatMap(c => c.sections || [])} 
            selectedIds={formData.sectionIds || []} 
            onChange={(ids) => setFormData(prev => ({ ...prev, sectionIds: ids }))}
          ...
          <MultiSelectDropdown 
            ...
            onChange={(ids) => setFormData(prev => ({ ...prev, subjectIds: ids }))}
            placeholder="Select Subjects..."
          />
        </div>
      </div>

      <AIAssistantCard message={`I've analyzed the current class schedules. The teacher is assigned to ${(formData.sectionIds || []).length} sections.`} />
