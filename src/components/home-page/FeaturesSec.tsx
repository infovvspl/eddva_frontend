...
export const FeatureSection: React.FC = () => {
  ...
  return (
    <main className="w-full min-h-screen bg-white text-slate-900 py-10 flex justify-center overflow-hidden">
      {/* Outer Container with w-7xl constraint */}
      <div className="w-full max-w-[80rem] flex flex-col h-screen relative">
        {/* SECTION HEADING */}
        <div className="w-full px-8 md:px-12 pt-8 md:pt-10 pb-4 md:pb-6 bg-white/90 backdrop-blur-md z-30 ">
          <motion.div
          ...
          >
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 bg-clip-text text-transparent">
              Our Platform's <span></span>
              <span
                style={{
        ...
        {/* Main Content Grid */}
        <div className="grid grid-cols-1 md:grid-cols-12 flex-1 relative">
          {/* LEFT PANEL: Sticky Visual Spotlight */}
          <div className="w-full md:col-span-5 md:sticky md:top-0 md:h-screen bg-white/90 backdrop-blur-md md:bg-white flex flex-col justify-between z-20 p-4 md:p-8 overflow-y-auto">
            {/* Center Content: Image + Text */}
            <div className="flex flex-col items-center text-center md:items-start md:text-left w-full space-y-6 md:space-y-8 my-auto">
              {/* Dynamic Image Container */}
              <div className="w-full max-w-xl md:max-w-none aspect-[4/3] md:aspect-[16/10] rounded-2xl overflow-hidden bg-slate-100 relative border border-slate-200 shadow-lg shrink-0">
              ...
              {/* Dynamic Typography - Changed from 'block md:hidden' to 'block' so it shows on all views */}
              <div className="space-y-3 md:space-y-5 w-full max-w-lg mx-auto md:mx-0">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={activeIdx}
                  ...
                  >
                    <motion.h3 className="text-xl md:text-2xl font-extrabold tracking-tight text-slate-900 leading-[1.2] md:leading-[1.1] bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent pb-2">
                      {currentFeature.title}
                    </motion.h3>
                    <motion.p
                      className="block md:hidden text-xs md:text-sm text-slate-600 leading-relaxed mt-2 md:mt-4 font-medium"
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    ...
                    {/* Feature accent line */}
                    <motion.div
                      className={`h-1 w-16 rounded-full mt-4 md:mt-6 ${currentFeature.accentBg.split(" ")[0].replace("bg-", "bg-")}`}
            ...
            {/* Footer Control Wrapper to keep controls tight on small heights */}
            <div className="w-full mt-6 md:mt-8 shrink-0">
              ...
              <div className="flex items-center justify-center gap-4 mt-4">
                <motion.button
                  ...
                  className="p-2.5 rounded-full bg-slate-100 hover:bg-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 shadow-sm hover:shadow"
                ...
                <motion.button
                  onClick={() =>
                    setActiveIdx((prev) =>
                      Math.min(FEATURES.length - 1, prev + 1),
                    )
                  }
                ...
                return (
                  ...
                  >
                    <motion.div
                      ...
                      className={`w-full p-6 md:p-14 rounded-3xl border transition-all duration-300 cursor-default group ${
